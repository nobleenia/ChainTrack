...
export default function CreateShipmentPage() {
    ...
    if (file) {
      ...
      if (!file.type.startsWith('image/')) {
        setValidationErrors((prev) => ({
          ...prev,
          pickup_photo: 'Please select an image file'
        }))
        return
      }
      ...
      if (file.size > 5 * 1024 * 1024) {
        setValidationErrors((prev) => ({
          ...prev,
          pickup_photo: 'Image must be less than 5MB'
        }))
        return
      }

      setFormData((prev) => ({ ...prev, pickup_photo: file }))
  ...
  const removePhoto = () => {
    setFormData((prev) => ({ ...prev, pickup_photo: null }))
  ...
  const validateForm = () => {
    ...
    if (!formData.pickup_photo) {
      errors.pickup_photo = 'Pickup photo is required for verification'
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Convert photo to base64 for now (will use IPFS later)
      let photoData = null
      if (formData.pickup_photo && photoPreview) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Take a clear photo of the package before shipping. This serves as proof of condition at pickup.
          </p>
          ...
          {validationErrors.pickup_photo && (
            <p className="text-red-500 text-sm mt-2">{validationErrors.pickup_photo}</p>
          )}
        </div>
